    ...
    <div ref={heroRef} className="relative w-full overflow-hidden text-[#0f172a] font-sans antialiased selection:bg-indigo-500 selection:text-white">
      ...
      <section className="relative pt-5 max-sm:pt-10 pb-14 px-6 mx-auto flex flex-col items-center text-center max-sm:px-6">
        ...
        <div className="inline-flex items-center gap-2 bg-indigo-50 border border-indigo-100 rounded-full px-1 py-1 pr-2 mb-8 shadow-sm">
          ...
          <span className="text-sm font-[450] text-indigo-950">Programs open for enrollment</span>
        </div>

        {/* Main Headline */}
        <h1 className="text-5xl md:text-7xl lg:text-[80px] font-bold text-gray-900 tracking-tight leading-[1.05] max-w-4xl mb-6">
          Move beyond your <br className="hidden sm:inline" /> learning curve
        </h1>

        {/* Subtitle */}
        <p className="text-gray-500 text-md md:text-md font-normal max-w-xl mb-10 leading-relaxed">
          Build real-world skills, work on meaningful projects, learn from industry
          professionals, and connect with a community built for ambitious careers.
        </p>
        ...
        <div className="relative w-full max-w-md max-sm:max-w-sm overflow-hidden py-4 my-2 [mask-image:linear-gradient(to_right,transparent,black_20%,black_80%,transparent)]">
          <div className="flex w-max gap-8 animate-marquee">
            {[...Array(4)].flatMap(() => [
              { label: "Real projects", icon: "🛠️", bg: "bg-amber-100", text: "text-amber-600" },
              { label: "Mentorship", icon: "🤝", bg: "bg-fuchsia-100", text: "text-fuchsia-600" },
              { label: "Community", icon: "🌍", bg: "bg-cyan-100", text: "text-cyan-600" },
            ]).map((item, idx) => (
              <div
                key={idx}
