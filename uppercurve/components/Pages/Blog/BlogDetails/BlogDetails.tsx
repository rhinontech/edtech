...
export function BlogDetails({ post }: { post: BlogPost }) {
  ...
  const morePosts = blogPosts.filter((p) => p.slug !== post.slug).slice(0, 4);
  ...
  useEffect(() => {
    const handleScroll = () => {
      const headingElements = headings.map((h) => ({
        ...
        el: document.getElementById(slugifyHeading(h.text)),
      }));
                ...
                {headings.map((heading) => {
                  ...
                  return (
                    <a
                      key={heading.text}
                      href={`#${headingId}`}
                      className={`text-[13.5px] leading-snug font-medium transition-colors block ${isActive
        ...
        <div className="mt-20 pt-10 border-t border-gray-200/80">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {morePosts.map((other, idx) => {
              ...
              const hasThumbnail = Boolean((other as any).thumbnail);
